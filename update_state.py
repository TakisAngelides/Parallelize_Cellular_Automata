...
@cuda.jit
def update_state_1D_r90(width, configurations_dev, iteration):
    x = cuda.grid(1)
    ...
    configurations_dev[iteration, x] = (
        (configurations_dev[iteration-1, x]) and (alive == 1)
    ) or ((not configurations_dev[iteration-1, x]) and (alive == 1))



@cuda.jit
def update_state_2D_rGOL(width, height, configurations_dev, iteration):
...
def update_state_2D_rTG(width, height, configurations_dev, iteration):
    ...
    configurations_dev[iteration, x, y] = ((configurations_dev[iteration-1, x, y]) or ((not configurations_dev[iteration-1, x, y])) and (alive >= 3) and (cuda.random.xoroshiro128p_uniform_float32() < 0.2 )) 



@cuda.jit
def update_state_3D_rCI(width, height, depth, configurations_dev, iteration):
    x, y, z = cuda.grid(3)
...
@cuda.jit
def update_state_3D_rBII(width, height, depth, configurations_dev, iteration):
    x, y, z = cuda.grid(3)
    ...
    configurations_dev[iteration, x, y, z] = (    (   (configurations_dev[iteration-1, x, y, z])    and  (alive <= 7 ) and (alive >= 5)) or (not configurations_dev[iteration-1, x, y, z] and alive == 1) )
